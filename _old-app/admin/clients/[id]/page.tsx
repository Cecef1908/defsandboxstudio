...
import type { ClientEntity, AdvertiserEntity, BrandEntity, ContactEntity } from "@/app/types/agence";
...
export default function ClientDetailPage() {
  ...
  async function fetchClientData() {
    try {
      ...
      const clientData = { id: clientDoc.id, ...clientDoc.data() } as ClientEntity;
      setClient(clientData);

      // 2. Récupérer les Annonceurs associés
      const advertisersQuery = query(
        collection(db, "advertisers"),
        where("client_id", "==", clientId)
      );
      const advertisersSnapshot = await getDocs(advertisersQuery);
      const advertisersData = advertisersSnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })) as AdvertiserEntity[];
  ...
  const getTypeLabel = (type: string) => {
    const types: Record<string, string> = {
      direct: "Annonceur Direct",
      agency: "Agence Média",
      holding: "Holding / Groupe"
    };
  ...
  if (!client) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-slate-400 text-lg">Client introuvable</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-slate-800">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="text-sm text-slate-500 mb-1">Type d'Entité</div>
                  <div className="text-slate-300">{getTypeLabel(client.type)}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Section 2 - Annonceurs (Départements) */}
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
          <div className="flex items-center gap-3 mb-4">
          ...
          {advertisers.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {advertisers.map((advertiser) => (
                <div
                  key={advertiser.id}
                  className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 hover:border-blue-500/50 transition-colors cursor-pointer"
            ...
            <div className="text-center py-8 text-slate-500">
              ...
              <p className="text-sm">Aucun annonceur lié à ce client</p>
            </div>
          )}
        </div>

        {/* Section 3 - Portefeuille de Marques */}
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
          <div className="flex items-center gap-3 mb-4">
          ...
          {brands.length > 0 ? (
            ...
            <div className="text-center py-8 text-slate-500">
              ...
              <p className="text-sm">Aucune marque liée à ce client</p>
            </div>
          )}
        </div>

        {/* Section 4 - Interlocuteurs (CRM) */}
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
          <div className="flex items-center gap-3 mb-4">
          ...
          {contacts.length > 0 ? (
            ...
            <div className="text-center py-8 text-slate-500">
              ...
              <p className="text-sm">Aucun contact lié à ce client</p>
            </div>
          )}
        </div>
